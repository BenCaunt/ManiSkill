"""Capture the actual Fill task port for independent host-side comparison."""
import argparse
import hashlib
import json
...
def run(args):
    ...
    env=FillEnv(mpm_device=args.device,obs_mode='none',reward_mode='dense',render_backend='gpu')
    records=[]
    try:
        env.reset(seed=args.seed)
        initial=snapshot(env)
        if len(initial['x']) != 704 or abs(float(initial['mass'].sum()) - .135168) > 1e-7:
            raise RuntimeError('Fill particle count or mass differs from the reference fixture')
        ...
        for i in range(args.steps):
            ...
            records.append({'step':i+1,'reward':float(reward[0]),'success':bool(info['success'][0]),
                            'contained_particles':int(info['contained_particles'][0]),
                            'spilled_particles':int(info['spilled_particles'][0])})
        ...
        body_data=[]
        for b in (env.bucket,env.beaker_body):
        ...
        return dict(completed=True,parity_validated=False,env_id='Fill-v0',seed=args.seed,steps=args.steps,
