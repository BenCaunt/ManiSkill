...
cooked_pack="$PWD/probe-inputs/cooked-probe-v5/pack"
if python3 -c 'import json,sys;sys.exit(0 if json.load(open(sys.argv[1])).get("prepared_wall_build") else 1)' "$input/request.json"; then
  cooked_pack=$(python3 - "$input/request.json" "$PWD" <<'PY_PACK'
import hashlib,json,pathlib,re,sys
request=json.loads(pathlib.Path(sys.argv[1]).read_text());label=request['prepared_wall_build']
assert re.fullmatch('wall-preparation-v[0-9]+',label)
root=pathlib.Path(sys.argv[2])/'records'/label/'prepared'
record=json.loads((root/'execution.json').read_text());pack=root/'pack'
expected=request['bottle_cooked_pack_sha256'];assert re.fullmatch('[a-f0-9]{64}',expected)
assert record['complete'] and record['pack_sha256']==expected
assert hashlib.sha256((pack/'pack.json').read_bytes()).hexdigest()==expected
print(pack)
PY_PACK
)
fi
...
while read -r case_index case_name task backend control_mode; do
  ...
  case "$task" in
    ...
    Pour)
      ...
      if [[ "$has_cooked" == yes ]]; then
        extra+=(-v "$cooked_pack:/cooked-pack:ro")
      fi ;;
