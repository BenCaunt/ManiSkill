    ...
    if os.name == 'nt':
        ...
        if (cuda_home and cu_path):
            ...
            with ScopedTimer("build_cuda", active=warp.config.verbose):
                ...
                ld_inputs.append(quote(cu_out))
                if os.path.isdir(os.path.join(cuda_home, "lib64")):
                    ld_inputs.append('-L"{cuda_home}/lib64" -lcudart_static -lnvrtc'.format(cuda_home=cuda_home))
                elif os.path.isdir(os.path.join(cuda_home, "lib")):
