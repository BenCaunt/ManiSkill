...
class DenseVolume:
    ...
    def __init__(self, data: np.ndarray, position: np.ndarray, rotation: np.ndarray, scale: np.ndarray, device="cuda"):
        self.id = 0
        ...
        if device == "cpu":
            ...
            self.id = self.context.core.dense_volume_create_host(
                ctypes.cast(data.__array_interface__["data"][0], ctypes.c_void_p),
                ctypes.c_uint64(x),
                ctypes.c_uint64(y),
                ctypes.c_uint64(z),
                ctypes.c_int(c),
            ...
            data = np.ascontiguousarray(
                data.transpose(2, 1, 0, 3).flatten(), dtype=np.float32
            )
            self.id = self.context.core.dense_volume_create_device(
                ctypes.cast(data.__array_interface__["data"][0], ctypes.c_void_p),
                ctypes.c_uint64(x),
                ctypes.c_uint64(y),
                ctypes.c_uint64(z),
    ...
    def __del__(self):
        ...
        try:
            ...
            if self.device == "cpu":
                self.context.core.dense_volume_destroy_host(self.id)
            else:
                self.context.core.dense_volume_destroy_device(self.id)
        except Exception as e:
            pass
