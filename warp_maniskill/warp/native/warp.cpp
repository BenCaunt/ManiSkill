...
// ManiSkill's dense-volume extension must export the same ABI on CPU builds.
WP_API uint64_t dense_volume_create_device(float*, uint64_t, uint64_t, uint64_t,
                                           int, const wp::vec3*, const wp::quat*,
                                           const wp::vec3*) { return 0; }
WP_API void dense_volume_destroy_device(uint64_t) {}
