...
// Pass a single pointer array across the Python/C boundary. The compiler handles
// the platform ABI for the mixed scalar and aggregate arguments internally.
WP_API void {name}_cpu_forward_packed(void** args)
{{
    {name}_cpu_forward({forward_packed});
}}

WP_API void {name}_cpu_backward_packed(void** args)
{{
    {name}_cpu_backward({reverse_packed});
}}
...
def codegen_module(kernel, device='cpu'):
    ...
    s = template.format(name=kernel.key,
                        ...
                        forward_packed=", ".join(
                            f"*static_cast<{t}*>(args[{i}])" for i, t in enumerate(
                                ["launch_bounds_t", *[a.ctype() for a in adj.args]])),
                        reverse_packed=", ".join(
                            f"*static_cast<{t}*>(args[{i}])" for i, t in enumerate(
                                ["launch_bounds_t", *[a.ctype() for a in adj.args], *[a.ctype() for a in adj.args]])))
    return s
