...
class Kernel:
    ...
    def input_types(self):
        if self.func:
            return [(a.label, a.type) for a in self.adj.args]
        else:
            return list(self.cls.input_types.items())
    ...
    def hook(self):
        ...
        if (dll):
            ...
            try:
                self.forward_cpu = eval("dll." + self.key + "_cpu_forward_packed")
                self.backward_cpu = eval("dll." + self.key + "_cpu_backward_packed")
            except:
...
def kernel(f):
    ...
    if inspect.isclass(f):
        return Kernel(func=None, key=warp.codegen.make_func_name(f), module=m, cls=f)
    return Kernel(func=f, key=warp.codegen.make_func_name(f), module=m)
...
class Module:
    ...
    def hash_module(self):
        ...
        h = hashlib.sha256()
        h.update(b"maniskill3-cpu-packed-abi-v1")
        ...
        for kernel in self.kernels.values():
            if kernel.func:
                ...
                s = kernel.cls.cpp_source
                h.update(bytes(s, 'utf-8'))
                s = kernel.cls.cuda_source
                h.update(bytes(s, 'utf-8'))
...
class Runtime:
    ...
    def __init__(self):
        ...
        self.core.dense_volume_create_host.argtypes = [ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
        self.core.dense_volume_create_host.restype = ctypes.c_uint64
        self.core.dense_volume_destroy_host.argtypes = [ctypes.c_uint64]

        self.core.dense_volume_create_device.argtypes = [ctypes.c_void_p, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_uint64, ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
        self.core.dense_volume_create_device.restype = ctypes.c_uint64
        self.core.dense_volume_destroy_device.argtypes = [ctypes.c_uint64]
...
def zeros(shape: Tuple=None, dtype=float, device: str="cpu", requires_grad: bool=False, **kwargs)-> warp.array:
    ...
    # NumPy scalar dimensions must become Python integers at the ctypes boundary.
    num_bytes = int(num_elements)*warp.types.type_size_in_bytes(dtype)
    ...
    if (bounds.size > 0):
        ...
        def pack_args(args, params):
            ...
            for i, a in enumerate(args):
                ...
                arg_name, arg_type = input_types[i]
        ...
        if (len(fwd_args)) != (len(kernel.input_types)):
        ...
        # run kernel
        if device == 'cpu':
            # Keep ctypes away from platform-specific aggregate argument ABIs.
            function = kernel.backward_cpu if adjoint else kernel.forward_cpu
            function.argtypes = [ctypes.POINTER(ctypes.c_void_p)]
            function.restype = None
            pointers = (ctypes.c_void_p * len(params))(*[ctypes.addressof(param) for param in params])
            function(pointers)
